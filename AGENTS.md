...
This guide specifies how to create new SVG icons for the `@marcusv/roc` library. Follow these rules exactly to maintain visual consistency across all styles and ensure the build pipeline works correctly.
...
| Import paths (React) | `@marcusv/roc/react/{style}` | `import { ArrowLeft } from '@marcusv/roc/react/outline'` |
| Import paths (Svelte) | `@marcusv/roc/svelte/{style}` | `import ArrowLeft from '@marcusv/roc/svelte/outline/ArrowLeft.svelte'` |
