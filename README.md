...
The generator uses local heuristics to group commit subjects into `Features`, `Fixes`, `Documentation`, `Build & Tooling`, `Icons`, and `Other Changes`. Output is deterministic: commit entries stay in `git log` order end-to-end, section blocks follow that same history order, empty sections are omitted, and each entry includes its short hash plus commit date. If history alternates between section types, the same section heading can appear more than once so the chronological order stays intact.
