...
Use the typed root React barrel:
...
> **Note**: Explicit React deep imports such as `@marcus/roc/react/outline/index.js` and `@marcus/roc/react/sharp/Search.jsx` resolve at runtime, but only `@marcus/roc/react/index.js` ships bundled TypeScript declarations today.
...
Use `npm run build` when you need a fresh [`demo/index.html`](demo/index.html) rebuild.
