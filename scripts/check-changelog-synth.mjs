...
function testClassificationHeuristics() {
  ...
  assert.equal(classifyCommit('feat: add package verification step'), 'Features');
  assert.equal(classifyCommit('feat: add docs browser'), 'Features');
  assert.equal(classifyCommit('Add changelog synthesizer CLI'), 'Features');
  ...
  assert.equal(classifyCommit('Add .DS_Store to .gitignore and remove from tracking'), 'Build & Tooling');
...
function testLiveHistorySmoke() {
  ...
  const synthesizerCommit = result.sections
    .flatMap((section) => section.entries.map((entry) => ({ ...entry, title: section.title })))
    .find((entry) => entry.subject === 'Add changelog synthesizer CLI');

  if (synthesizerCommit) {
    assert.equal(
      synthesizerCommit.title,
      'Features',
      'expected the synthesizer commit to appear under Features when present in live history',
    );
  }
}
