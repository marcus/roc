...
function testClassificationHeuristics() {
  ...
  assert.equal(
    classifyCommit('build: add package verification step'),
    'Build & Tooling',
  );
...
function testSectionFormatting() {
  const sections = buildSections([
    ...
    },
    {
      hash: '3',
      shortHash: '3333333',
      subject: 'Add 12 new icon variants',
      date: '2026-04-17',
    },
  ]);
  ...
  assert.equal(
    cleanSubject('feat: add changelog generator (td-123abc)'),
...
function testLiveHistorySmoke() {
  ...
  const result = generateChangelog({
    repo,
    limit: 12,
    title: 'Smoke Test Changelog',
  });

  assert.ok(
    result.commits.length > 0,
    'expected at least one commit in live history',
  );
  assert.ok(
    result.sections.length > 0,
    'expected at least one rendered section',
  );
  assert.match(result.markdown, /^# Smoke Test Changelog/m);
  assert.ok(
    result.markdown.includes(
      '_Source: full repository history through `HEAD`._',
    ),
