...
const packOut = execSync('npm pack --pack-destination . 2>/dev/null', {
  cwd: ROOT,
  encoding: 'utf8',
}).trim();
...
const sampleIcons = fs
  .readdirSync(path.join(ROOT, 'dist/svelte/outline'))
...
for (const name of sampleIcons) {
  lines.push(
    `const _check_${name}: Component<{ size?: number }> = ${name}Outline;`,
  );
}
lines.push('');
