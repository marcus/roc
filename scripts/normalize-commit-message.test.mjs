...
test('rewrites the first non-empty line before a Git-generated help block', async () => {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), 'roc-commit-message-'));
  const filePath = path.join(tempDir, 'COMMIT_EDITMSG');
  const originalMessage = `  chore:   tighten   subject spacing!  \n
Body stays put.
...
test('supports alternate configured comment prefixes when Git generated the help block', () => {
  const originalMessage = `  docs:   update   README   \n
; Please enter the commit message for your changes. Lines starting
; with ';' will be ignored, and an empty message aborts the commit.
;
; On branch main
`;
  ...
  assert.notEqual(findGitCommentBlockStart(originalMessage, ';'), null);
  assert.equal(
...
test('does not treat comment-prefixed non-interactive subjects as ignorable', () => {
  const originalMessage = `#123 fix bug

body line
`;

  assert.equal(normalizeMessage(originalMessage), originalMessage);
});

test('does not ignore alternate comment-prefixed subjects without a Git help block', () => {
  const originalMessage = `; lowercase subject
...
test('does not infer auto comment prefixes from arbitrary punctuation lines', () => {
  const originalMessage = `; lowercase subject
...
`;
  ...
  assert.equal(inferCommentPrefix(originalMessage), null);
  assert.equal(resolveCommentPrefix('auto', originalMessage), null);
  assert.equal(normalizeMessage(originalMessage, { commentPrefix: 'auto' }), originalMessage);
});
