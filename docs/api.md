...
| Import path | Resolves to | TypeScript support | Notes |
| --- | --- | --- | --- |
| `@marcus/roc/react/index.js` | `dist/react/index.js` | Yes | Preferred React entry point |
| `@marcus/roc/react/{style}/index.js` | `dist/react/{style}/index.js` | No bundled `.d.ts` | Runtime export only |
| `@marcus/roc/react/{style}/{Icon}.jsx` | `dist/react/{style}/{Icon}.jsx` | No bundled `.d.ts` | Runtime export only |
...
These paths resolve at runtime:
...
They do not ship matching declaration files in the current package. In strict TypeScript projects, they produce `TS7016`. Keep consumer-facing TypeScript examples on the root React barrel until the package gains typed React subpath declarations.
