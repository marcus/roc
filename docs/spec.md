...
- `--demo` reuses the current source SVGs and metadata to rebuild only the gallery
...
`./react/*` is exported with a wildcard, so React subpaths resolve through the package export map. The generated files are still `.jsx`, which means bare Node ESM cannot execute them without a JSX-aware loader or bundler.

The current package also only ships a declaration file for `dist/react/index.d.ts`, not for style barrels or per-icon `.jsx` files.

That means:

- `@marcus/roc/react/index.js` is the typed React entry point
- all React entry points are meant for React bundlers and frameworks that transpile JSX
- explicit React file imports such as `@marcus/roc/react/{style}/index.js` and `@marcus/roc/react/{style}/{Icon}.jsx` are published but do not have bundled TypeScript declarations

Consumer docs must keep that distinction explicit until the package starts generating React subpath declarations and stops depending on raw `.jsx` execution in downstream toolchains.
...
- [`CONTRIBUTING.md`](../CONTRIBUTING.md) is the contributor setup and verification guide
...
5. Confirm React docs describe the exports as JSX-toolchain-facing and do not imply typed support for untyped deep imports
